...
const programPath = path.join(config.repoRoot, "apps", "api", "src", "cli", "snapshot.ts");
...
<dict>
  ...
  <string>${escapeXml(path.join(logDir, "collector.out.log"))}</string>
  <key>StandardErrorPath</key>
  <string>${escapeXml(path.join(logDir, "collector.err.log"))}</string>
</dict>
</plist>
`;
